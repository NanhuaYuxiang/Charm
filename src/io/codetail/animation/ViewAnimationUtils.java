...
public class ViewAnimationUtils {
	 ...
	 * @param centerX
	 *            The x coordinate of the center of the animating circle.
	 * @param centerY
	 *            The y coordinate of the center of the animating circle.
	 * @param startRadius
	...
	public static SupportAnimator createCircularReveal(View view, int centerX,
		...
		ObjectAnimator reveal = ObjectAnimator.ofFloat(revealLayout,
				"revealRadius", startRadius, endRadius);
	...
	static Animator.AnimatorListener getRevealFinishListener(
			RevealAnimator target, Rect bounds) {
		if (SDK_INT >= 17) {
	 ...
	 * @param baseRotation
	 *            initial Rotation X in 3D space
	 * @param fromY
	 ...
	 */
	public static void liftingFromBottom(View view, float baseRotation,
			float fromY, int duration, int startDelay) {
		ViewHelper.setRotationX(view, baseRotation);
		ViewHelper.setTranslationY(view, fromY);

		ViewPropertyAnimator.animate(view)
				.setInterpolator(new AccelerateDecelerateInterpolator())
				.setDuration(duration).setStartDelay(startDelay).rotationX(0)
	...
	public static class SimpleAnimationListener implements
