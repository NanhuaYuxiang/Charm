 ...
 * 
 */
public class GenerateFaceModel extends BaseActivity implements OnClickListener {
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == REQUEST_CAMERA_IMAGE) {
			...
			if (null != bmp) {
				FaceUtil.saveBitmapToFile(GenerateFaceModel.this, bmp);
			}
			// 获取图片保存路径
			fileSrc = FaceUtil.getImagePath(GenerateFaceModel.this);
			// 获取图片的宽和高
	...
	private void register(JSONObject obj) throws JSONException {
		...
		if ("success".equals(obj.get("rst"))) {
			showTip("注册成功");
			// 上传完model需要在本地标记一下，这样下次就不需要拍照上传model
			SharedPreferences.Editor editor=getSharedPreferences(SettingActivity.IS_OPEN_FACE_VERIFY,
					MODE_PRIVATE).edit();
			editor.putBoolean("isModelExist", true);
			editor.commit();
		} else {
			showTip("注册失败");
		}
